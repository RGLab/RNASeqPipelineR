...
createProject <- function(project_name,path=".",verbose=FALSE, load_from_immport=FALSE){
  ...
  if(success)
    invisible()
  cmnd_prefix <- "mkdir -p "
  dirs <- c(SRA="SRA",FASTQ="FASTQ",RSEM="RSEM",FASTQC="FASTQC",GEO="GEO",CONFIG="CONFIG",OUTPUT="OUTPUT",RAWANNOTATIONS="RAW_ANNOTATIONS",RNASEQC="RNASEQC",TOPHAT="TOPHAT", BAM='BAM', KALLISTO='KALLISTO')
...
#' Build a index for reference transcriptome 
#'  
#' Builds a index reference transcriptome at `Utils/Reference_Genome`
#' 
#' You must specify the Utils path if it is not already defined, and have your genome in a folder titled
#' `Reference_Genome`. This function will construct the index reference transcriptome using kallisto tools.
#' The command line is the default shown in the documentation.
#' @param path \code{character} specifying an \emph{absolute path} path to the Utils directory.
#' @param transcripts_fasta_file \code{character} the name of the transcript fasta file, must be specified. 
#' We used the transcripts.fa file generated by RSEM rsem-prepare-reference step. 
#' @param name \code{character} the name of the genome output.
#' @export
buildIndex <- function(path=NULL, transcripts_fasta_file=NULL,name=NULL){
  if(is.null(transcripts_fasta_file)|is.null(name))
    stop("You must provide a fasta_file  and a genome name")
  if(is.null(path)&inherits(try(getConfig()[["subdirs"]][["Utils"]],silent=TRUE),"try-error")){
    stop("Please specify where to build the reference genome")
  }else if(is.null(path)){
  ...
  if(substr(path, 1, 1) != '/') stop("'path' must be an absolute path")
  #check if the reference genome has already been built
  if(length(list.files(pattern=paste0(name,".transcripts.idx"),path=file.path(getConfig()[["subdirs"]]["Utils"],"Reference_Genome")))==0){
    command = paste0("kallisto index -i ", path, "/", name, ".transcripts.idx ", path, "/", transcripts_fasta_file)
    system(command)
  }else{
    message("Reference Genome Found")
  }
...
#' Use the Kallisto tool to align the reads and calcuate the count and tpm values for each sample
#' 
#' Uses Kallisto to align reads in FASTQ files against the reference genome. Optionally you can specify paired end reads. The code assumes 
#' paired reads have fastq files that differ by one character (i.e. sampleA_read1.fastq, sampleA_read2.fastq) and will perform
#' matching of paired fastq files based on that assumption using string edit distance. Read 1 is assumed to be upstream
#' and read 2 is assumed to be downstream. 
#' The number of parallel_threads should not be more than the number of cores available on your system.
#' @param parallel_threads \code{integer} specify how many parallel processes to spawn
#' @param paired \code{logical} specify whether you have paried reads or not.
#' @param nchunks \code{integer} number of chunks to split the files for a slurm job. Ignored if slurm = FALSE
#' @param slurm \code{logical} if \code{TRUE} job is submitted as a slurm batch job, otherwise it's run on the local machine. Slurm jobs will honour the nchunks and days_requested arguments. 
#' @param slurm_partition \code{character} the slurm partition to submit to. Ignored if slurm=FALSE
#' @export
KALLISTOCalculateExpression <- function(parallel_threads=1, paired=TRUE,nchunks=10,slurm=FALSE,slurm_partition="gottardo_r"){
  #Chunking for slurm
  library(stringr)
  library(ShortRead)
  ncores<-parallel_threads
  if(ncores>parallel::detectCores()&!slurm){
    stop("The number of parallel_threads*bowite_threads is more than the number of cores detected by detectCores() on the local machine for non-slurm jobs")
  }
  .chunkDataFrame<-function(df=pairs,nchunks=nchunks){        
  ...
  kallisto_dir <- getConfig()[["subdirs"]][["KALLISTO"]]
  done <- list.dirs(kallisto_dir, full.names=FALSE, recursive=FALSE)
  ...
  index <- list.files(paste0(getConfig()[["subdirs"]][["Utils"]],"/Reference_Genome"), ".transcripts.idx$", full=TRUE, recursive=FALSE)
  keep <- outer(keep, extension, paste0)
  ...
  if(slurm){
    ...
    writeLines(c("#!/bin/bash",
                 ...
                 "#SBATCH -N 1                 # Ensure that all cores are on one machine",
                 paste0("#SBATCH -p ",slurm_partition,"    # Partition to submit to"),
                 paste0("#SBATCH -o ",file.path(getConfig()[["subdirs"]][["FASTQ"]],"kallisto_%a.out"),"      # File to which STDOUT will be written"),
                 paste0("#SBATCH -e ",file.path(getConfig()[["subdirs"]][["FASTQ"]],"kallisto_%a.err"), "     # File to which STDERR will be written")),con=con)
    argumentFile <- file.path(getConfig()[["subdirs"]][["FASTQ"]], "arguments_chunk_${SLURM_ARRAY_TASK_ID}.txt")
  } else{
    #If we aren't using slurm, set the number of chunks to one.
    nchunks<-1
  ...
  if(!paired){
    ...
    aveLength <- unlist(mclapply(myfiles[,1], function(x){
      mean(width(readFastq(x)))
    }, mc.cores=parallel_threads))
    myfiles <- cbind(myfiles, aveLength)
    command<-paste0("cd ",kallisto_dir," && parallel -n 3 -j ",parallel_threads," kallisto quant -i ", index, " -b 20 -l {3} -o {2} {1} :::: ", argumentFile)
  }else{
    #get file names, commandline for Paired end
    fastq_files<-file.path(fastq_dir,sort(keep))
    pairs<-matrix(fastq_files,ncol=2,byrow=TRUE)
    myfiles<-cbind(pairs,gsub("\\.fastq","",gsub("_[12]\\.","\\.",basename(pairs[,1]))))
    command<-paste0("cd ",kallisto_dir," && parallel -n 3 -j ",parallel_threads," kallisto quant -i ", index, " -o {3} -b 20 {1} {2} :::: ", argumentFile)
  }
...
#' @export
#' @import annotate AnnotationDbi
KALLISTOBioCAnnotate <- function(annotation_library="TxDb.Hsapiens.UCSC.hg38.knownGene",force=FALSE,lib="org.Hs.eg.db"){
  cond_eval <- length(list.files(getConfig()[["subdirs"]][["KALLISTO"]], pattern="kallisto_"))<3
  if(cond_eval|force){
    message("Assembling counts matrix")
    kallisto_files <- list.files(getConfig()[["subdirs"]][["KALLISTO"]], pattern="abundance.txt", full=T, recursive=T)
    # Read all files and create a list of data.tables
    kallisto_list <- lapply(kallisto_files, function(x) data.table(fread(x), samples=sapply(strsplit(x,"/"), function(s) s[length(s)-1]))) 
    # Bind all the data.tables to create a long data.table
    kallisto_data_long <- rbindlist(kallisto_list)
    
    # Rename a column, so that it's a bit R friendly
    setnames(kallisto_data_long, "target_id", "transcript_id")
