...
#' @param slurm if \code{TRUE} job is submitted as a slurm batch job, otherwise it's run on the local machine. Slurm jobs will honour the nchunks and days_requested arguments.
...
#' @param mail email address to send failure message to to, if desired.
#' @export
  ...
  if(slurm){
    ...
    runKallisto <- function(fastqName) {
      kalliCommand <- paste0("cd ", fastq_dir, " && kallisto quant -i ", ref_full_path,
                             ...
                             fastqName, paired_pattern[2], " 2>&1 | tee ", kallisto_dir, 
